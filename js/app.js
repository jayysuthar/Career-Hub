...
function renderCalendar() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const count = (visits[dateStr] || []).filter((id) =>
      companies.some((c) => c.id === id),
    ).length;
  ...
  // Summary row
  const monthVisitDays = Object.keys(visits).filter((k) =>
    k.startsWith(viewingStr),
  ).length;
  html += `<p class="cal-summary">${
    monthVisitDays > 0
...
function openEmailModal(preselectedCompanyId = null, editEmailId = null) {
  const existing = editEmailId
    ? emails.find((e) => e.id === editEmailId)
    : null;
  ...
  select.innerHTML =
    ...
    companies
      .map(
        (c) =>
          `<option value="${c.id}"${c.id === companyId ? " selected" : ""}>${esc(c.name)}</option>`,
      )
      .join("");
  ...
  document.getElementById("email-modal-title").textContent = existing
    ? "Edit contact"
    : "Add contact";
  document.getElementById("email-modal-save").textContent = existing
    ? "Save changes"
    : "Add contact";
...
function saveEmailForm() {
  ...
  let ok = true;
  if (!companyId) {
    document.getElementById("err-ef-company").hidden = false;
    ok = false;
  } else {
    document.getElementById("err-ef-company").hidden = true;
  }
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    document.getElementById("err-ef-email").hidden = false;
    ok = false;
  } else {
    document.getElementById("err-ef-email").hidden = true;
  }
  if (phone) {
    ...
    if (digits.length !== 10) {
      document.getElementById("err-ef-phone").hidden = false;
      ok = false;
    } else {
      document.getElementById("err-ef-phone").hidden = true;
    }
...
function onDrop(e) {
  ...
  if (dragSrcId && dragSrcId !== this.dataset.id)
    moveCompany(dragSrcId, this.dataset.id);
}

function onDragEnd() {
  document
    .querySelectorAll(".card--dragging, .card--drag-over")
...
function renderTagPills() {
  ...
  tags.forEach((tag) =>
    container.appendChild(btn(tag, tag, filterTag === tag)),
  );
  ...
  panel.innerHTML = `
    ...
    <div class="contact-table-wrap">
      <table class="contact-table">
        ...
        : '<p class="card-emails-empty">No contacts yet.</p>'
    }
  ...
  try {
    ...
    if (Array.isArray(data.companies)) {
      for (const comp of data.companies) {
        if (!comp.url || !comp.name) continue;
        if (companies.some((c) => c.url === comp.url)) {
          skippedCompanies++;
          continue;
        }
        companies.push({
...
document
  .getElementById("email-modal-cancel")
  .addEventListener("click", closeEmailModal);
document
  .getElementById("email-modal-save")
  .addEventListener("click", saveEmailForm);
document
  .getElementById("email-modal-overlay")
...
document.getElementById("import-file").addEventListener("change", (e) => {
  ...
  reader.onload = (ev) => {
    ...
    if (file.name.endsWith(".json")) {
      result = importFromJSON(ev.target.result);
    } else {
...
document
  ...
  .addEventListener("click", closeLinkedInModal);
document
  .getElementById("linkedin-modal-cancel")
  .addEventListener("click", closeLinkedInModal);
document
  .getElementById("linkedin-modal-save")
  .addEventListener("click", saveLinkedIn);
document
  .getElementById("linkedin-modal-overlay")
  .addEventListener("click", (e) => {
